...
const { GitHub } = require('@actions/github/lib/utils.js');
  ...
  try {
    ...
    for (const step in pipeline) {
      ...
      try { //run step
        ...
        error_msg = "Error! Skipping remaining steps.\n" + 
        ...
        if (core.getInput("ContinueOnFail") === true)
          core.warning("In pipeline " + process.env.GITHUB_WORKFLOW + ":\n" + error_msg);
        else core.error("In pipeline " + process.env.GITHUB_WORKFLOW + ":\n" + error_msg);
      }
    }
